...
"	mngv*sqrt(2 * 6.67 * 1000000 * 7.34767309 / (1*length(from+dp-cent)/0.5*1737) ))*(0.5/6371)*p+dp;\r\n"
//"	return ( ngv*sqrt(2 * 6.67 * 1000000 * 7.34767309 / (1*length(from+dp-cent)/0.5*1737))*(0.5/1737)*p+dp );// * s;\r\n"
...
"	cent = ep;\r\n"
"	//cent.z *= -1;\r\n"
"	vec3 to = outpos.x * right * width / height + outpos.y * up + view;\r\n"
"	vec3 from = camcen;\r\n"
"	vec3 dp = to - from;\r\n"
"	dp = dp / length(dp*155);\r\n"
"	float i; float f = 0;\r\n"
...
"		//if(color.w > 0.0)\r\n"
"		//{\r\n"
"		f = f + (1.0 - outcolor.w) * 1.0 / 300.0 / 3.0;"
...
"	//gl_FragColor = vec4(vec3(1,sa,2)/length(vec3(1,sa,2)), 1); gl_FragDepth = 0.5;\r\n"
#endif
"	//return ( ngv*sqrt(2 * 6.67 * 1000000 * 7.34767309 / (1*length(from+dp-cent)/0.5*1737))*(0.5/6371)*p+dp );// * s;\r\n"
"	return ( engv*sqrt(2 * 6.67 * 100000000 * 5.9722 / (1*length(from+dp-cent)/0.5*6371)) + \r\n"
"	ngv*sqrt(2 * 6.67 * 1000000 * 7.34767309 / (1*length(from+dp-cent)/0.5*1737) ))*(0.5/6371)*p+dp;\r\n"
"}\r\n"
"vec4 trylook(vec3 from, vec3 to, vec3 cent)\r\n"
"{\r\n"
"	float fd = length(cent - from);\r\n"
